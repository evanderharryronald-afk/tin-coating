...
#
#     # 设置残差异常阈值 (例如超出 2 倍标准差)
#     top_threshold = 2 * top_delta_std
#     bot_threshold = 2 * bot_delta_std
#
...
# # 3. 残差建模核心类
# # ==========================================
# # 网格搜索结论（见 grid_search 实验记录）：
# #   - damping 越大（越追求方向平衡），整体 RMSE 单调变差，
# #     Top/Bot 两个表面均无例外，说明"完全平衡两个方向"在本数据集上没有自由午餐。
# #   - 不加权（damping=0）+ alpha_smoothing≈0.7 时，两个表面 RMSE 均接近网格最优，
# #     是当前默认的生产配置。
# #   - 若业务上必须保证"偏低方向不能比在线仪表更差"（如涉及质量红线），
# #     可以把 damping 调到 0.6~0.8 附近，用 RMSE 的下降换取该方向不被拖累，
# #     属于业务取舍，不是技术上能兼得的问题。
...
#     - 目标量方差小、接近0，避免树模型重建高相关恒等映射引入的额外噪声；
#     - 对'在线测量值'这一特征施加单调约束，防止修正方向局部错乱；
#     - 预测残差做 EWMA 平滑，抑制预测噪声引起的震荡；
#     - 方向样本加权作为可选项保留（默认 damping=0 即关闭），
#       仅在业务明确要求"某方向不能变差"时按需开启。
#     """
#
...
#     如需切换到"两方向都不变差"的保守解，可参考：
#         Top 表面：网格搜索未找到双赢组合，若必须保守可尝试 damping=0.2~0.3
#         Bot 表面：damping=0.7, alpha_smoothing=0.7 为双赢解（RMSE=0.125，比默认配置0.106略差）
#     """
#     prefix = 'Top' if surface == 'Top' else 'Bot'
#     surface_cn = '上' if surface == 'Top' else '下'
#
...
#     # 如需切换为"偏低方向绝不能变差"的保守方案，可参考函数注释里的备选参数。
...
#
#     # 备选：若业务上必须保证下表面"偏低方向"不能比在线仪表更差，可改用以下保守配置替代默认调用：
#     # run_surface_pipeline(clean_df, surface='Bot', damping=0.7, alpha_smoothing=0.7)
import os
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
...
def analyze_correlations(df, surface='Top'):
    ...
    actual_col = f'Tin Weight_Actual[g/m2]_GALV_WEIGHT_{prefix.upper()}_Avg'
    # actual_col = f'Tin Weight_Actual[g/m2]_GALV_WEIGHT_{prefix.upper()}_Min'
    lab_col = f'{surface_cn}表面镀层重量A(XA1_0)'
