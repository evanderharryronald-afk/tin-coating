...
#     if unmatched:
#         warn(f"主表中有 {len(unmatched)} 个关键字段值在副表中无匹配，"
#              f"这些行的副表列将填入空值。")
#         if len(unmatched) <= 10:
...
#
#     log(f"\n✅ 全部完成！输出文件: {out_path}")
#     log(f"   结果表: {len(result_df)} 行 × {len(result_df.columns)} 列")
#
#
