...
#     wb_full.close()
#
#     # 🌟 核心改动：从同级目录读取独立的列名更正配置文件 rename_patch.yaml
...
#     """
#     带本地文件缓存的表头结构获取函数。配置或独立更正文件变更时自动更新。
#     """
#     cache_path = base_dir / "master_schema_cache.json"
#     patch_path = base_dir / "rename_patch.yaml"
#
#     # 获取 rename_patch.yaml 的内容快照，确保证典一改，缓存能立刻感知并失效刷新
#     patch_content = ""
#     if patch_path.exists():
...
#
#     master_numeric = pd.api.types.is_numeric_dtype(master_df[master_key_col].dropna())
#     sub_numeric = pd.api.types.is_numeric_dtype(sub_df[sub_key_col].dropna())
#     if master_numeric != sub_numeric:
#         die(f"关键字段类型不一致：主表 '{master_key_col}' 为 {'数值' if master_numeric else '文本'}型，"
#             f"副表 '{sub_key_col}' 为 {'数值' if sub_numeric else '文本'}型。")
#
...
#
#     sub_keep_df = sub_df[[sub_key_col] + [c for c, _ in processed_keep]]
#
...
#             target_name = f"{col_name}_sub"
#             warn(f"副表列 '{col_name}' 与主表列重名，重命名为 '{target_name}'。")
#             agg_df = agg_df.rename(columns={col_name: target_name})
#
#         if col_name in required_cols_cfg:
#             final_required_cols.append(target_name)
#
...
#         log(f"   已删除 {len(master_df) - len(result)} 行未匹配记录，保留 {len(result)} 行。")
#         if all_null_action == 'drop':
#             sub_cols_in_result = [c for c in agg_df.columns if c != sub_key_col]
#             before = len(result)
...
#
#     if final_required_cols:
#         before_req = len(result)
#         result = result.dropna(subset=final_required_cols, how='any')
#         dropped_req = before_req - len(result)
#         if dropped_req > 0:
#             log(f"   ⚠️ 校验 required_columns 规则：因特定必填列 {required_cols_cfg} 存在空值，进一步剔除了 {dropped_req} 行，保留 {len(result)} 行。")
#
...
#
#     # 🌟 传入 base_dir。函数内部不仅会做缓存验证，还会监听 rename_patch.yaml 的变化
